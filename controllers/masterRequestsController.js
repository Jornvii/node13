...
const createMasterRequest = async (req, res) => {
  try {
    const db = await connectDb();
    const { Part_ID, Rev_ID, Process_ID, Division_ID, Emp_ID, Date_Req, Date_Stamp, Use_for } = req.body;
    ...
    await db.query(query, [Part_ID, Rev_ID, Process_ID, Division_ID, Emp_ID, Date_Req, Date_Stamp, Use_for]);
